...
create procedure createAccount() begin
    ...
    if not exists (select * FROM information_schema.columns WHERE table_schema = DATABASE()  AND table_name = 'kb_accounts') then
        ...
        alter table kb_accounts comment '账号';

        alter table kb_accounts add constraint FK_Reference_14 foreign key (level_id)
            references kb_levels (id) on delete restrict on update restrict;

        alter table kb_accounts add constraint FK_Reference_5 foreign key (organization_id)
            references kb_organizations (id) on delete restrict on update restrict;

        alter table kb_accounts add constraint FK_Reference_6 foreign key (staff_id)
            references kb_staffs (id) on delete restrict on update restrict;
