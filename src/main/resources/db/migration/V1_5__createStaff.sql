...
create procedure createStaff() begin
    ...
    if not exists (select * FROM information_schema.columns WHERE table_schema = DATABASE()  AND table_name = 'kb_staffs') then
        /*==============================================================*/
        /* Table: kb_staffs                                             */
        /*==============================================================*/
        create table kb_staffs
        (
        ...
        alter table kb_staffs comment '员工信息';

        alter table kb_staffs add constraint FK_Reference_15 foreign key (major_id)
            references kb_majors (id) on delete restrict on update restrict;

        alter table kb_staffs add constraint FK_Reference_2 foreign key (organization_id)
            references kb_organizations (id) on delete restrict on update restrict;

        alter table kb_staffs add constraint FK_Reference_3 foreign key (department_id)
            references kb_departments (id) on delete restrict on update restrict;
